...
import prac.Sender;
import prac.FileChooserUtil;
import prac.FileSaveLoadUtil;
import prac.HTMLMaker;
import prac.UserList;
import prac.ChatCommand;
import prac.Message;
...
public class ChatPanel extends JPanel {
	...
	JTextPane roomTextPane;
	JTextArea txtrMessage;
	HTMLDocument doc;
	DefaultListModel<String> userListModel = new DefaultListModel<String>();
	DefaultListModel<String> roomListModel = new DefaultListModel<String>();
	...
	private JFrame emoticonFrame = new Emoticon();
	...
	public ChatPanel() {		
		...
		chatBoardPane.setBounds(0, 0, 436, 386);
		add(chatBoardPane);
		chatBoardPane.setLayout(null);
		
		chatScrollPane = new JScrollPane();
		chatScrollPane.setHorizontalScrollBarPolicy(ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
		chatScrollPane.setBounds(0, 45, 436, 341);
		...
		roomScrollPane.setBounds(0, 45, 436, 341);
		chatBoardPane.add(roomScrollPane);
		...
		roomTextPane.setFont(new Font("맑은 고딕", Font.PLAIN, 12));
		roomTextPane.setBackground(Color.PINK);
		roomScrollPane.setViewportView(roomTextPane);
		...
		JLabel lblUserList = new JLabel("\uCE5C\uAD6C\uBAA9\uB85D");
		...
		scrollPane.setBounds(10, 396, 298, 70);
		add(scrollPane);
		...
		btnNewButton.setBounds(359, 396, 65, 35);
		add(btnNewButton);
		
		JLabel lblImage = new JLabel(new ImageIcon("Image/image.png"));
		...
		lblImage.setBounds(346, 441, 30, 30);
		add(lblImage);
		
		JLabel lblSave = new JLabel(new ImageIcon("Image/save.png"));
		...
		lblSave.setBounds(381, 441, 30, 30);
		add(lblSave);		
		
		JLabel lblEmoticon = new JLabel(new ImageIcon("Image/emo.png"));
		...
		lblEmoticon.setBounds(320, 401, 30, 30);
		add(lblEmoticon);
		
		JLabel lblRoomList = new JLabel("방목록");
		lblRoomList.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent arg0) {
				roomListControl();
			}
		});
		lblRoomList.setFont(new Font("맑은 고딕", Font.BOLD, 36));
		lblRoomList.setHorizontalAlignment(SwingConstants.CENTER);
		lblRoomList.setBounds(384, 0, 40, 40);
		chatBoardPane.add(lblRoomList);
		roomTextPane.setContentType("text/html");
		doc = (HTMLDocument) roomTextPane.getStyledDocument();
		
		roomList = new JList(roomListModel);
		roomList.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				new Thread(new Listener(null)).start();
			}
		});
		roomList.setBackground(Color.WHITE);
		roomList.setFont(new Font("맑은 고딕", Font.PLAIN, 12));
		roomScrollPane.setColumnHeaderView(roomList);
		roomList.setVisible(false);
		roomList.setVisibleRowCount(0);
	...
	private void setRoomList() {
		roomListModel.clear();
		roomListModel.addElement(LoginPanel.Room); 
	}
	...
	static void sendEmoticon(int index) {
		String emoticonPath = Emoticon.EMOTICONPATH + "/icon"+index+".png";
		Sender.getSender().sendImage(emoticonPath);
	}
	...
	public void addMessage(boolean isMine, Message message) {
		...
		Sender.insert_DB(message.getName(), message.getMessage());
	}
	
	private void rewriteChatPane() {
		chatTextPane.setText(messageList.toString());
		chatTextPane.setCaretPosition(doc.getLength());
		roomTextPane.setText(messageList.toString());
		roomTextPane.setCaretPosition(doc.getLength());
	}
