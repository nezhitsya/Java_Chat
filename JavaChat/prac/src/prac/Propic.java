...
import server.MemberChange;
// 프로필 사진 프레임
public class Propic extends JFrame {

	public static final String PROFILEPATH = "Image/profile";
		...
		for (int i = 0; i < numberOfProfile; i++) {
			...
			lblProfile[i].addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent event) {
					...
					Design.changeProfileImage(Integer.parseInt(profile.getText()));
					Design.chProfileImage(Integer.parseInt(profile.getText()));
					MemberChange.chanProfileImage(Integer.parseInt(profile.getText()));
					dispose();
				}
			});
